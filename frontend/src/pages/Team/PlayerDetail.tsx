...
function PlayerDetail() {
  ...
  return (
    <div className="max-w-[1200px]">
      ...
      {/* 球员信息头部 */}
      <Card className="mb-6 bg-gradient-to-br from-[#0D4A4D]/30 to-[#12121A]">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-[#0D7377] to-[#0A5A5D] border border-[#0D7377]/50 flex items-center justify-center text-6xl shadow-lg shadow-[#0D7377]/20">
            👤
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-3xl font-bold text-white">{player.display_name}</h1>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getPositionColor(player.primary_position)}`}>
          ...
          <div className="flex items-center gap-6">
            <div className="text-center">
              <div className="text-4xl font-bold stat-number text-[#0D7377]">{player.overall_rating}</div>
              <div className="text-xs text-[#8B8BA7]">当前能力</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-bold stat-number text-emerald-400">{player.potential}</div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-4 rounded-lg bg-[#1E1E2D]">
                <p className="text-xs text-[#8B8BA7] mb-1">周薪</p>
                <p className="font-bold stat-number text-white">€{(player.wage / 1000).toFixed(0)}K</p>
              </div>
              <div className="p-4 rounded-lg bg-[#1E1E2D]">
                <p className="text-xs text-[#8B8BA7] mb-1">合同到期</p>
                <p className="font-bold stat-number text-white">{new Date(player.contract_end).toLocaleDateString('zh-CN')}</p>
              </div>
              <div className="p-4 rounded-lg bg-[#1E1E2D]">
                <p className="text-xs text-[#8B8BA7] mb-1">解约金</p>
                <p className="font-bold stat-number text-emerald-400">€{(player.release_clause / 1000000).toFixed(1)}M</p>
              </div>
              <div className="p-4 rounded-lg bg-[#1E1E2D]">
                <p className="text-xs text-[#8B8BA7] mb-1">市场价值</p>
                <p className="font-bold stat-number text-[#0D7377]">€{(player.market_value / 1000000).toFixed(1)}M</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-4 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-white">{player.matches_played}</div>
                <div className="text-xs text-[#8B8BA7]">出场</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-emerald-400">{player.goals}</div>
                <div className="text-xs text-[#8B8BA7]">进球</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-[#0D7377]">{player.assists}</div>
                <div className="text-xs text-[#8B8BA7]">助攻</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-amber-400">{player.average_rating}</div>
                <div className="text-xs text-[#8B8BA7]">场均评分</div>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-[#2D2D44] space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#8B8BA7]">黄牌</span>
                <span className="font-bold stat-number text-yellow-400">{player.yellow_cards}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#8B8BA7]">红牌</span>
                <span className="font-bold stat-number text-red-400">{player.red_cards}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#8B8BA7]">出场时间</span>
                <span className="font-bold stat-number">{player.minutes_played} 分钟</span>
              </div>
            </div>
          </Card>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold stat-number">{player.fitness}%</span>
                </div>
                <div className="h-2 bg-[#1E1E2D] rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full ${player.fitness > 80 ? 'bg-emerald-500' : player.fitness > 50 ? 'bg-amber-500' : 'bg-red-500'}`}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold stat-number">{player.morale}%</span>
                </div>
                <div className="h-2 bg-[#1E1E2D] rounded-full overflow-hidden">
                  <div 
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold stat-number">{player.form}%</span>
                </div>
                <div className="h-2 bg-[#1E1E2D] rounded-full overflow-hidden">
                  <div 
          ...
          <Card>
            ...
            <div className="p-4 rounded-lg bg-gradient-to-br from-[#0D4A4D]/30 to-[#12121A] border border-[#0D7377]/30">
