    ...
    <div className="max-w-[1200px]">
      ...
      <Card className="mb-6 bg-gradient-to-br from-[#0D4A4D]/30 to-[#12121A]">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-[#0D7377] to-[#0A5A5D] border border-[#0D7377]/50 flex items-center justify-center text-5xl shadow-lg shadow-[#0D7377]/20">
            🐉
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="text-center">
              <div className="text-3xl font-bold stat-number text-[#0D7377]">{team.overall_rating}</div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold stat-number">{team.attack}</span>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold stat-number">{team.midfield}</span>
                </div>
                <div className="h-2 bg-[#1E1E2D] rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-[#0D7377] to-[#14A085] rounded-full" 
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="font-bold stat-number">{team.defense}</span>
                </div>
                <div className="h-2 bg-[#1E1E2D] rounded-full overflow-hidden">
                  <div 
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center p-3 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-white">{team.stats.matches_played}</div>
                <div className="text-xs text-[#8B8BA7]">场次</div>
              </div>
              <div className="text-center p-3 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-emerald-400">{team.stats.wins}</div>
                <div className="text-xs text-[#8B8BA7]">胜</div>
              </div>
              <div className="text-center p-3 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-[#8B8BA7]">{team.stats.draws}</div>
                <div className="text-xs text-[#8B8BA7]">平</div>
              </div>
              <div className="text-center p-3 rounded-lg bg-[#1E1E2D]">
                <div className="text-2xl font-bold stat-number text-red-400">{team.stats.losses}</div>
            ...
            <div className="mt-4 pt-4 border-t border-[#2D2D44]">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold stat-number">{team.stats.goals_for} / {team.stats.goals_against}</span>
              </div>
              <div className="flex items-center justify-between mt-2">
                <span className="text-sm text-[#8B8BA7]">积分</span>
                <span className="font-bold stat-number text-[#0D7377]">{team.stats.points}</span>
              </div>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold stat-number text-emerald-400">
                  €{(team.finances.balance / 1000000).toFixed(1)}M
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#8B8BA7]">周薪支出</span>
                <span className="font-bold stat-number text-red-400">
                  €{(team.finances.weekly_wages / 1000).toFixed(0)}K
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#8B8BA7]">球场容量</span>
                <span className="font-bold stat-number">{team.finances.stadium_capacity.toLocaleString()}</span>
              </div>
            </div>
          </Card>
        </div>
                  ...
                  {MOCK_PLAYERS.map(player => (
                    <tr key={player.id} className="border-b border-[#2D2D44] hover:bg-[#1E1E2D]/50 transition-colors">
                      <td className="py-3 px-2">
                        <span className={`inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-bold ${POSITION_COLORS[getPositionGroup(player.position)]}`}>
                          {player.position}
                        </span>
                      </td>
                      <td className="py-3 px-2">
                      ...
                      <td className="py-3 px-2 text-center">
                        <span className={`font-bold stat-number ${
