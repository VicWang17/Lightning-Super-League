...
| --- | --- | --- |
| `GK` | `0.0018` | 门将跑动少，疲劳对初始体力影响较小。 |
| `DF` | `0.0025` | 后卫中等。 |
| `MF` | `0.0030` | 中场覆盖大，受疲劳影响最大。 |
| `FW` | `0.0028` | 前锋冲刺多，影响高于后卫。 |

示例：

| 球员 | 体力 | 疲劳 | 位置 | 初始体力折扣 |
| --- | --- | --- | --- | --- |
| 门将 | 95 | 50 | GK | `1 - 50*0.0018 = 91%` |
| 后卫 | 95 | 50 | DF | `87.5%` |
| 中场 | 95 | 50 | MF | `85%` |
| 前锋 | 95 | 50 | FW | `86%` |
...
initial_stamina = 82.8
```

玩家可以理解为“体力看起来很满，但长期疲劳让他只能带着约 83 的状态开场”。
...
```
中场出场 70 分钟，杯赛淘汰赛：
fatigue_delta = 13 * 1.10 * 1.10 = 15.73 -> 16
fitness_delta = -14
```
...
> 调整思路：降低单次训练和比赛的疲劳增量，提高自然恢复速度，让“常规训练 + 合理轮换”的球员长期保持较低疲劳；疲劳只会在连续高强度训练 + 频繁满勤比赛时累积到高位。
...
公式最终还会叠加 `decline_factor`，表示年龄带来的自然成长或衰退偏移：
...
晚熟且 `growth_peak_age >= 30` 的球员衰退更慢：
...
设计目标：90 OVR 左右的精英球员如果进入 35 岁仍未退役，核心属性应明显下滑，综合能力通常应落到 70 多到 80 出头区间；早熟、爆发型球员会更早衰退。
...
| --- | --- |
| 1 周 | 关键属性提升 0.6-1.5。 |
| 1 赛季 42 天 | 关键属性提升 3.0-6.0。 |
| 2-3 赛季 | 明显从替补成长为主力或轮换核心。 |
...
| 时间 | 预期 |
| --- | --- |
| 1 周 | 28-31 岁微调，33+ 开始出现可见衰退。 |
| 1 赛季 | 31-32 岁小幅下降，33-34 岁明显下降，35+ 快速下降。 |

### 20.2 单次训练建议范围

| 情况 | 单次主属性成长 |
| --- | --- |
| 高潜年轻球员，适配训练 | 0.18-0.45 |
| 普通年轻球员，适配训练 | 0.08-0.20 |
| 成年主力，适配训练 | 0.02-0.06 |
| 31-32 岁老将 | -0.08-0.03 |
| 35+ 老将 | -0.25 至 -0.10 |
| 接近上限 | 0.00-0.10 |
