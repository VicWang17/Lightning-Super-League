...
- 青训球员在营内自动训练和成长。
- 17-18 岁青训球员允许签入一线队；17 岁初始能力略低于 18 岁，但成长速度倾向更好。
...
- 青训/选秀签约使用 `contract_type=ROOKIE`，年限限制 1-2；默认值建议 2。
...
- 青训刷新年龄 17-18（Rookie 仅 17-18 岁）。
- 年龄仍由 `birth_offset` 和当前赛季号计算。
- 17-18 岁允许签入一线队；17 岁生成时初始 OVR 略低于 18 岁，优势体现在成长速度倾向更好。
- 未签约球员赛季末进入 `rookie_protected` 自由市场保护池。v1 不做跨季留营，降低流程复杂度。
...
- 每次刷新最多填满空位。
- 生成年龄 17-18 的青训球员（Rookie 仅 17-18 岁）。
...
| --- | --- | --- | --- |
| `<= 10%` | 18 岁更多 | C/D 更多 | 30-44 |
| `11%-17%` | 17/18 均衡 | B/C 更多 | 32-47 |
| `18%-25%` | 17 岁更多 | A/B 更多，少量 S | 34-50 |
...
| 项 | 值 |
| --- | --- |
| `base_growth` | 0.12-0.28 属性点/次 |
| fast | 1.30 |
| normal | 1.00 |
| slow | 0.70 |
| age 17 | 1.00 |
| age 18 | 0.90 |
| `random_factor` | 0.85-1.15 |

属性增长不能超过 `potential_max` 对应上限。实现优先提升该位置权重高的属性，少量随机提升其他属性。
...
- 默认年限 2，可选 1-2。
- 建议工资 = 普通建议工资 * 0.55-0.70（当前代码按 estimate_initial_wage * 0.55 生成 listing，青训签约预览使用 ContractService ROOKIE 系数）。
...
当前 v1 实现：未签约青训球员进入 `FreeAgentListing(origin="academy_released", rookie_protected=true)` 自由市场保护池，而不是独立的选秀池。后续版本可再接入 `draft_pool` 选秀大会流程。
...
4. 选中第一名可用球员。
5. 若志愿内无可用球员，则按默认池补选。
6. 球队若 roster 已满 18，跳过本队选择，并发送邮件提醒“阵容已满，无法参与本次选秀签约”。

选中后动作：

- 创建 `draft_selections(status=pending)`。
- 玩家进入选秀结果页或球员详情页，点击签约入口进入新人合同签约页。
- 默认 `contract_type=ROOKIE`、默认 2 年、可选 1-2 年、建议工资为正常建议工资的 0.70。
...
- 玩家球队若未处理，系统尝试自动签约。
- 自动签约条件：roster `< 18`、球队余额足够支付签字费或签约即时成本、工资帽/财务限制允许、球员仍未被签走且未退役。
...
- roster 未满 18 且球员潜力 A/S，倾向签约。
- 17 岁球员在成长速度 fast 或潜力 A/S 时优先签。
...
- 全 AI 联赛跑 2-3 个赛季后，各队 roster 仍在 8-18 人内。
...
1. 17-18 岁青训球员允许直接签入一线队；17 岁初始能力略低于 18 岁，成长速度倾向更好。
