    ...
    <div className="p-4 rounded-xl bg-[#12121A] border border-[#2D2D44] hover:border-[#0D7377]/30 transition-colors">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {stageConfig && (
            <span className={`text-xs px-2 py-0.5 rounded-full bg-gradient-to-r ${stageConfig.color} text-white`}>
              {stageConfig.name}
            </span>
          )}
        </div>
  ...
  return (
    <tr className="border-b border-[#2D2D44] hover:bg-[#1E1E2D]/50 transition-colors">
      ...
      <td className="py-2 px-3 text-center">
        <span className="font-bold stat-number">{points}</span>
      </td>
    </tr>
  )
}
...
function GroupSection({ group }: { group: CupGroup }) {
  ...
  return (
    <div className="rounded-xl bg-[#12121A] border border-[#2D2D44] overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-[#0D7377]/20 to-transparent border-b border-[#2D2D44]">
...
function TreeMatchCard({ match, showTBD }: { match?: CupFixture; showTBD?: boolean }) {
  if (showTBD || !match) {
    return (
      <div className="w-28 p-2 rounded-lg bg-[#1A1A25]/50 border border-dashed border-[#3D3D55]">
  ...
  return (
    <div className="w-28 p-2 rounded-lg bg-[#1A1A25] border border-[#2D2D44] hover:border-[#0D7377]/50 transition-colors">
      <div className="space-y-1">
  ...
  return (
    <div className="overflow-x-auto pb-4">
        ...
        <div className="flex" style={{ minHeight: `${60 + 16 * 60}px` }}>
          {stageOrder.map((stage, stageIndex) => {
            ...
            return (
              ...
              >
                {/* 阶段标题 */}
                <div className={`mb-4 px-3 py-1 rounded-full bg-gradient-to-r ${config.color} text-white text-xs font-medium whitespace-nowrap`}>
      ...
      <div className="mt-4 flex items-center justify-center gap-6 text-xs text-[#8B8BA7]">
        ...
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded bg-[#1A1A25] border border-dashed border-[#3D3D55]" />
          <span>待定</span>
        </div>
      </div>
    </div>
      ...
      <div className="card mb-6 bg-gradient-to-br from-[#0D4A4D]/30 to-[#12121A]">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${isLightningCup ? 'from-yellow-500 to-amber-500' : 'from-emerald-500 to-teal-500'} flex items-center justify-center text-3xl shadow-lg`}>
              {config.icon}
            </div>
            <div>
          ...
          <div>
            ...
            {groupsLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="h-48 rounded-xl bg-[#1E1E2D] animate-pulse" />
                ))}
              </div>
            ) : groups.length === 0 ? (
              <div className="text-center py-12">
        ...
        {activeTab === 'knockout' && (
          <div>
            ...
            <div>
              ...
              {fixturesLoading ? (
                <div className="h-64 rounded-xl bg-[#1E1E2D] animate-pulse" />
