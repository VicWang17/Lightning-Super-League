...
export default function RookieMarket() {
  ...
  const resetFilters = () => {
    setFilters({ position: '', min_ovr: '', max_ovr: '', min_age: '', max_age: '' })
    setPage(1)
  }
  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      <PageHeader
        title="新人市场"
        subtitle="赛季末未被签约的青训球员会进入这里，带有新人保护期"
      />

      <YouthTabs />

      {/* 提示 */}
      <div className="p-3 bg-emerald-500/10 border-2 border-emerald-500/30">
        <p className="text-sm text-emerald-400">
          这里的球员来自各队青训营，年龄 17-18 岁。带有"新人保护期"标签的球员只能由原母队联赛内球队签约（当前 v1 仅作展示）。
        </p>
      </div>

      {/* 筛选面板 */}
      <div className="p-4 bg-[#12121A] border-2 border-[#2D2D44]">
      ...
      {loading && (
        <div className="flex items-center justify-center py-12 text-sm text-[#8B8BA7]">
          <Loader className="w-5 h-5 text-[#0D7377] animate-spin mr-2" />
          加载中...
        </div>
      )}
      {error && (
        ...
        <>
            ...
            {players.map((p) => (
              <div key={p.listing_id} className="card card-hover">
                ...
                <div className="flex items-center gap-2 mb-3">
                  <Badge variant="success" size="sm">青训新人</Badge>
                  {p.growth_speed && (
                    <span className="text-xs px-1.5 py-0.5 bg-[#2D2D44] text-[#8B8BA7]">
                      {growthSpeedLabels[p.growth_speed] || p.growth_speed}速成长
                    </span>
                  )}
                  {p.is_rookie_protected && (
                    <Badge variant="primary" size="sm">新人保护期</Badge>
                  )}
                </div>
          ...
          {players.length === 0 && (
            <div className="text-center py-12 text-[#8B8BA7]">
              <p className="text-sm">暂无符合条件的新人球员</p>
            </div>
          )}
