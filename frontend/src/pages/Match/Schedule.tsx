    ...
    `}>
      ...
      <div className="col-span-2">
        <div className="flex items-center gap-2">
          ...
          {isCurrentDay && (
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#0D7377] text-white">
              今天
            </span>
          )}
        </div>
      ...
      <div className="col-span-2 flex items-center justify-center gap-3">
        {fixture.status === 'scheduled' ? (
          ...
          <div className="flex items-center gap-3">
            <span className={`text-lg font-bold ${isHomeMyTeam ? 'text-white' : 'text-[#8B8BA7]'}`}>
              {fixture.home_score}
            </span>
            <span className="text-[#4B4B6A]">:</span>
            <span className={`text-lg font-bold ${isAwayMyTeam ? 'text-white' : 'text-[#8B8BA7]'}`}>
              {fixture.away_score}
            </span>
          </div>
        )}
      ...
      <div className="col-span-1 flex items-center justify-end gap-2">
        {resultBadge && (
          <span className={`text-xs px-2 py-0.5 rounded border ${resultBadge.className}`}>
            {resultBadge.text}
          </span>
        )}
        <span className={`text-xs px-2 py-0.5 rounded border ${statusConfig.pillClassName}`}>
          {statusConfig.label}
        </span>
      </div>
    </div>
  ...
  if (loading) {
    return (
      <div className="max-w-[1200px]">
        <div className="h-8 w-48 rounded bg-[#1E1E2D] animate-pulse mb-4" />
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-24 rounded-xl bg-[#1E1E2D] animate-pulse" />
          ))}
        </div>
        <div className="h-96 rounded-xl bg-[#1E1E2D] animate-pulse" />
      </div>
    )
  }
    ...
    <div className="max-w-[1200px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="card p-4">
          ...
          <p className="text-2xl font-bold stat-number text-white">{stats.played}</p>
        </div>
        ...
        <div className="card p-4">
          ...
          <p className="text-2xl font-bold stat-number text-white">
